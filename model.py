...
def preprocess_data(data):                                 #convert categorical variables to numerical
...
if __name__ == "__main__":         
    ...
    df=pd.crosstab(data['AGE'],data['Risk1Yr']) #how many patients of each age group died within 1 year or survived
    print(df)
    ...
    # Initialize and train a logistic regression model ,SVC and Random Forest model
    lgr_model = LogisticRegression(max_iter=1000)         #Logistic Regression Classifier
    ...
    svc_model = svm.LinearSVC(dual=False, max_iter=10000)  # Use LinearSVC for better performance on large datasets
    ...
    lreg_gs=GridSearchCV(LogisticRegression(random_state=42),
                         log_reg_grid,
                          cv=5,
                        scoring='f1' or 'recall' or 'roc_auc',
                        verbose=True,
                        )
...
# prediction function that takes patient input data and returns risk prediction
